 ...
 * @method     ChildLyricQuery orderBymanualCensore($order = Criteria::ASC) Order by the manual_censore column
 ...
 * @method     ChildLyricQuery groupBymanualCensore() Group by the manual_censore column
 ...
 * @method     ChildLyric findOneBymanualCensore(boolean $manual_censore) Return the first ChildLyric filtered by the manual_censore column
 ...
 * @method     ChildLyric requireOneBymanualCensore(boolean $manual_censore) Return the first ChildLyric filtered by the manual_censore column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildLyric[]|ObjectCollection findBymanualCensore(boolean $manual_censore) Return ChildLyric objects filtered by the manual_censore column
...
abstract class LyricQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `id`, `title`, `text`, `text_bg`, `text_bg_added`, `extra_info`, `send_by`, `cache_title_short`, `cache_censor`, `manual_censore`, `cache_censor_updated`, `views`, `popularity`, `votes_count`, `video_youtube`, `video_vbox7`, `video_metacafe`, `download` FROM `lyric` WHERE `id` = :p0';
        try {
    ...
    /**
     * Filter the query on the manual_censore column
     *
     * Example usage:
     * <code>
     * $query->filterBymanualCensore(true); // WHERE manual_censore = true
     * $query->filterBymanualCensore('yes'); // WHERE manual_censore = true
     * </code>
     *
     * @param     boolean|string $manualCensore The value to use as filter.
     *              Non-boolean arguments are converted using the following rules:
     *                * 1, '1', 'true',  'on',  and 'yes' are converted to boolean true
     *                * 0, '0', 'false', 'off', and 'no'  are converted to boolean false
     *              Check on string values is case insensitive (so 'FaLsE' is seen as 'false').
     ...
     * @return $this|ChildLyricQuery The current query, for fluid interface
     */
    public function filterBymanualCensore($manualCensore = null, $comparison = null)
    {
        if (is_string($manualCensore)) {
            $manualCensore = in_array(strtolower($manualCensore), array('false', 'off', '-', 'no', 'n', '0', '')) ? false : true;
        }

        return $this->addUsingAlias(LyricTableMap::COL_MANUAL_CENSORE, $manualCensore, $comparison);
    }
