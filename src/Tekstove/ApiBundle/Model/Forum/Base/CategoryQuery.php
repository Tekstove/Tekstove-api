 ...
 * @method     ChildCategoryQuery orderByOrder($order = Criteria::ASC) Order by the order column
 ...
 * @method     ChildCategoryQuery groupByOrder() Group by the order column
 ...
 * @method     ChildCategory findOneByOrder(int $order) Return the first ChildCategory filtered by the order column
 * @method     ChildCategory findOneByHidden(int $hidden) Return the first ChildCategory filtered by the hidden column *
 ...
 * @method     ChildCategory requireOneByOrder(int $order) Return the first ChildCategory filtered by the order column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildCategory requireOneByHidden(int $hidden) Return the first ChildCategory filtered by the hidden column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildCategory[]|ObjectCollection findByOrder(int $order) Return ChildCategory objects filtered by the order column
 * @method     ChildCategory[]|ObjectCollection findByHidden(int $hidden) Return ChildCategory objects filtered by the hidden column
...
abstract class CategoryQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Tekstove\ApiBundle\Model\Forum\Base\CategoryQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\Tekstove\\ApiBundle\\Model\\Forum\\Category', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT `id`, `name`, `order`, `hidden` FROM `forum_category` WHERE `id` = :p0';
        try {
    ...
    public function filterByOrder($order = null, $comparison = null)
    {
        if (is_array($order)) {
            $useMinMax = false;
            if (isset($order['min'])) {
                $this->addUsingAlias(CategoryTableMap::COL_ORDER, $order['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($order['max'])) {
                $this->addUsingAlias(CategoryTableMap::COL_ORDER, $order['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(CategoryTableMap::COL_ORDER, $order, $comparison);
    }
    ...
    public function filterByHidden($hidden = null, $comparison = null)
    {
        if (is_array($hidden)) {
            $useMinMax = false;
            if (isset($hidden['min'])) {
                $this->addUsingAlias(CategoryTableMap::COL_HIDDEN, $hidden['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($hidden['max'])) {
                $this->addUsingAlias(CategoryTableMap::COL_HIDDEN, $hidden['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
